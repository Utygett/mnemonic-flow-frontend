...
1) Закрепить табовый shell в `widgets/main-shell` (public api).
2) Перенести реализацию из временных мест и убрать legacy-папки.
3) Везде перейти на Public API (`.../index.ts`), запретить deep-imports.
